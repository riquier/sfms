    ...
    describe('gitDiffFilesOnly', () => {
        it('can handle changes', () => {
            child_process.exec.mockResolvedValueOnce({
                stdout: 'A       force-app/main/default/flows/DEX_Circumstances_SCORE_Assessment.flow-meta.xml'
            });
            const output = gitDiffFilesOnly();
            expect(output).resolves.toEqual('A       force-app/main/default/flows/DEX_Circumstances_SCORE_Assessment.flow-meta.xml');
